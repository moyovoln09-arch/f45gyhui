...
hysteria2://pass@host:443?up=200&down=200&sni=example.com&obfs=salamander&obfs-password=pass#name
```
...
- `--shuffle`: Случайный порядок тестирования ключей.
- `--shuffle-output`: Перемешать топ-результаты в консоли.
- `--export-config path.json`: Экспортировать конфигурацию `sing-box` с балансировкой (random balancer) между лучшими узлами.
- `--show-inbound`: Показать пример настройки Hysteria2 на стороне сервера.
