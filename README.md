...
hysteria2://password@host:443?sni=example.com&obfs=salamander&obfs-password=123#name
```
...
## Рандомизация и экспорт
- `--shuffle`: перемешивает входные ключи перед тестом.
- `--export-singbox`: создает готовый конфиг для `sing-box`.
- В экспортированном конфиге ноды перемешиваются, а по умолчанию используется `loadbalance` с типом `random`, чтобы трафик распределялся равномерно и не было смещения в сторону "первого ключа".
- Hysteria2 по умолчанию использует 200 Мбит/с (up/down), что оптимально для игр.
