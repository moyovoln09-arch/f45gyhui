...
def parse_link(link: str) -> Tuple[str, Dict]:
    ...
    if scheme == "vless":
        ...
        outbound = {
            ...
            "up_mbps": int(qs.get("up", qs.get("up_mbps", 200))),
            "down_mbps": int(qs.get("down", qs.get("down_mbps", 200))),
        }
        if qs.get("obfs"):
            outbound["obfs"] = {
                "type": qs.get("obfs"),
    ...
    if security in {"tls", "reality"} or scheme == "hysteria2":
        tls = {
            ...
            "insecure": (
                qs.get("allowInsecure", "0") in {"1", "true", "True"} or
                qs.get("insecure", "0") in {"1", "true", "True"}
            ),
        }
        if security == "reality":
            tls["reality"] = {
