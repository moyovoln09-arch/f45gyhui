...
def parse_link(link: str) -> Tuple[str, Dict]:
    ...
    if scheme == "vless":
        ...
        up = qs.get("up_mbps") or qs.get("up")
        down = qs.get("down_mbps") or qs.get("down")
        outbound["up_mbps"] = to_int(up, 200)
        outbound["down_mbps"] = to_int(down, 200)

        tls = {
            "enabled": True,
            "server_name": qs.get("sni") or server,
            "insecure": qs.get("insecure", "0") in {"1", "true", "True"}
            or qs.get("allowInsecure", "0") in {"1", "true", "True"},
        }
        outbound["tls"] = tls

        if qs.get("obfs"):
            outbound["obfs"] = {"type": qs["obfs"], "password": qs.get("obfs-password", "")}
...
def export_singbox_config(
    results: List[TestResult], output_path: Path, shuffle_output: bool = True
) -> None:
    ...
    for idx, res in enumerate(working, start=1):
...
def main() -> None:
    ...
    parser.add_argument("--export-singbox", help="Path to export best working keys as sing-box config")
    parser.add_argument("--no-shuffle-output", action="store_true", help="Disable shuffling of nodes in exported config")
    ...
    if args.export_singbox:
        ...
        export_singbox_config(
            top_results, Path(args.export_singbox), shuffle_output=not args.no_shuffle_output
        )
