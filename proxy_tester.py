...
def ensure_dependencies(auto_install: bool) -> None:
    ...
    if missing_bins and auto_install:
        ...
        if shutil.which("apt-get") and shutil.which("sudo"):
            ...
            run_cmd(cmd)
            to_install = [b for b in missing_bins if b != "yt-dlp"]
            if to_install:
...
def parse_link(link: str, default_bandwidth: int = 200) -> Tuple[str, Dict]:
    parsed = urlparse(link.strip())
    scheme = parsed.scheme.lower()
    if scheme not in {"vless", "trojan", "hysteria2", "hy2", "hysteria"}:
    ...
    if scheme == "vless":
        ...
        outbound = {
            ...
            "server": server,
            "server_port": port,
            "auth_str": unquote(parsed.username) if parsed.username else qs.get("auth", ""),
            "up_mbps": to_int(qs.get("up") or qs.get("up_mbps"), default_bandwidth),
            "down_mbps": to_int(qs.get("down") or qs.get("down_mbps"), default_bandwidth),
        }
        if qs.get("obfs"):
            outbound["obfs"] = qs["obfs"]
        ...
        if qs.get("obfs"):
            outbound["obfs"] = {
                "type": qs["obfs"],
                "password": qs.get("obfs-password") or qs.get("obfs_password", ""),
            }

    security = qs.get("security", "")
    # Hysteria/Hysteria2 always uses TLS in sing-box, but we can set SNI and insecure
    if security in {"tls", "reality"} or scheme in {"hysteria", "hysteria2", "hy2"}:
        tls = {
            "enabled": True,
            "server_name": qs.get("sni") or qs.get("host") or server,
            "insecure": (qs.get("allowInsecure") or qs.get("insecure") or "0") in {"1", "true", "True"},
        }
        if security == "reality":
            tls["reality"] = {
                "enabled": True,
...
def evaluate_key(key: str, timeout_s: int, sites: List[str], magnet: str, yt_url: str, default_bandwidth: int = 200) -> TestResult:
    with tempfile.TemporaryDirectory(prefix="keytest_") as td:
        td_path = Path(td)
        cfg_path = td_path / "config.json"

        try:
            protocol, outbound = parse_link(key, default_bandwidth)
...
def export_singbox_config(
    results: List[TestResult], top_n: int, output_path: Path, shuffle_output: bool = True, default_bandwidth: int = 200
) -> None:
    ranked = sorted(results, key=lambda r: r.total_score, reverse=True)
    top_results = [r for r in ranked[:top_n] if r.working]
    if not top_results:
        print("[!] No working nodes to export.")
        return

    # Extract outbound configs and shuffle if requested
    nodes_to_export = []
    for r in top_results:
        _, outbound = parse_link(r.key, default_bandwidth)
        nodes_to_export.append(outbound)

    if shuffle_output:
        random.shuffle(nodes_to_export)

    # Assign sequential tags to hide performance ranking
    for idx, node in enumerate(nodes_to_export, start=1):
        node["tag"] = f"node-{idx}"

    node_tags = [n["tag"] for n in nodes_to_export]
    ...
    config = {
        ...
        "dns": {
            ...
            "rules": [{"outbound": "any", "server": "google"}, {"query_type": ["A", "AAAA"], "server": "google"}],
        },
        "inbounds": [
            {
                "type": "mixed",
        ...
        "outbounds": [
            {
                "type": "selector",
                "tag": "select",
                "outbounds": ["random", "auto"] + node_tags,
            },
            {
                "type": "urltest",
                "tag": "auto",
    ...
    print(f"[+] Sing-box configuration exported to: {output_path}")
...
def print_hysteria2_inbound_example() -> None:
    config = {
        "type": "hysteria2",
        "tag": "hy2-in",
        "listen": "::",
    ...
    print("\n=== HYSTERIA2 INBOUND EXAMPLE (sing-box) ===")
    print(json.dumps(config, indent=2))
    print("\n=== AUTO-SETUP COMMAND (Ubuntu/Debian) ===")
    print(
        "curl -fsSL https://raw.githubusercontent.com/apernet/hysteria/master/install.sh | bash"
    )


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Test VLESS/Trojan/Hysteria/Hysteria2 keys and rank best 5 by average score (sites + YouTube + torrent + stability)."
    )
    parser.add_argument("--keys", required=True, help="Path to text file with vless://, trojan://, hysteria:// or hy2:// links")
    ...
    parser.add_argument("--export-singbox", help="Path to export the final sing-box configuration")
    parser.add_argument("--shuffle", action="store_true", help="Shuffle input keys before testing")
    parser.add_argument("--no-shuffle-output", action="store_true", help="Disable shuffling nodes in exported config")
    parser.add_argument("--hy-bandwidth", type=int, default=200, help="Default Hysteria/Hysteria2 bandwidth in Mbps")
    parser.add_argument("--print-hy2-example", action="store_true", help="Print Hysteria2 server inbound example and exit")
    parser.add_argument("--auto-install", action="store_true", help="Try to install missing dependencies")
    args = parser.parse_args()

    if args.print_hy2_example:
        print_hysteria2_inbound_example()
        return
    ...
    for i, key in enumerate(keys, start=1):
        ...
        res = evaluate_key(key, args.timeout, DEFAULT_SITES, args.magnet, args.youtube_url, args.hy_bandwidth)
        results.append(res)
    ...
    if args.export_singbox:
        export_singbox_config(
            results, args.top, Path(args.export_singbox), not args.no_shuffle_output, args.hy_bandwidth
        )
