...
def parse_link(link: str) -> Tuple[str, Dict]:
    ...
    if scheme == "vless":
        ...
        outbound = {
            ...
            "up_mbps": to_int(qs.get("up") or qs.get("up_mbps"), 200),
            "down_mbps": to_int(qs.get("down") or qs.get("down_mbps"), 200),
        }
        if qs.get("obfs"):
            outbound["obfs"] = {
                "type": qs["obfs"],
...
def main() -> None:
    ...
    parser.add_argument(
        "--keys", required=True, help="Path to text file with vless://, trojan:// and hy2:// links"
    )
