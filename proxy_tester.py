...
def parse_link(link: str, default_bandwidth: int = 200) -> Tuple[str, Dict]:
    ...
    if scheme not in {"vless", "trojan", "hysteria", "hysteria2", "hy2"}:
    ...
    if scheme == "vless":
        ...
        outbound = {
            ...
            "server": server,
            "server_port": port,
            "auth_str": unquote(parsed.username or qs.get("auth") or ""),
        ...
        if qs.get("obfs"):
            outbound["obfs"] = {
                "type": qs["obfs"],
                "password": qs.get("obfs-password", qs.get("obfsParam", "")),
            }

    security = qs.get("security", "")
    if security in {"tls", "reality"} or scheme in {"hysteria", "hysteria2"}:
        insecure = qs.get("insecure") or qs.get("allowInsecure")
        tls = {
            "enabled": True,
            "server_name": qs.get("sni") or qs.get("host") or server,
            "insecure": str(insecure).lower() in {"1", "true"},
        }
        if security == "reality":
            tls["reality"] = {
                "enabled": True,
            ...
            if qs.get("fp"):
                tls["utls"] = {"enabled": True, "fingerprint": qs["fp"]}
        outbound["tls"] = tls
...
def evaluate_key(
    ...
    with tempfile.TemporaryDirectory(prefix="keytest_") as td:
        ...
        try:
            protocol, outbound = parse_link(key, default_bandwidth=default_bandwidth)
...
def print_top(results: List[TestResult], top_n: int, shuffle_output: bool = False) -> None:
    ranked = sorted(results, key=lambda r: r.total_score, reverse=True)
    top_keys = ranked[:top_n]
    if shuffle_output:
        random.shuffle(top_keys)

    print("\n=== TOP {} KEYS ({}order) ===".format(top_n, "shuffled " if shuffle_output else ""))
    for idx, r in enumerate(top_keys, start=1):
        print(
...
def export_singbox_config(
    ...
    ranked = sorted([r for r in results if r.working], key=lambda r: r.total_score, reverse=True)[
        :top_n
    ]
    if not ranked:
    ...
    for r in ranked:
        try:
            _, outbound = parse_link(r.key, default_bandwidth=default_bandwidth)
    ...
    print(f"[+] Sing-box config exported to: {path}")
...
def print_hysteria2_inbound_example() -> None:
    ...
    print("\n=== HYSTERIA2 INBOUND EXAMPLE (for your server) ===")
    print(json.dumps(inbound, indent=2))
    print("\n--- Quick Setup Script (Panel-like automation) ---")
    print("# Run this on your server to quickly setup Hysteria2 with sing-box:")
    print("cat <<EOF > config.json")
    print(json.dumps({"log": {"level": "info"}, "inbounds": [inbound], "outbounds": [{"type": "direct"}]}, indent=2))
    print("EOF")
    print("sing-box run -c config.json")

    print("\n--- 3X-UI Panel API Example ---")
    print("# If you use 3X-UI, you can add this inbound via API:")
    print("curl -X POST http://YOUR_PANEL_IP:PORT/panel/api/inbounds/add \\")
    print("     -H 'Content-Type: application/json' \\")
    print("     -d '{")
    print("       \"enable\": true,")
    print("       \"remark\": \"Hysteria2-Auto\",")
    print("       \"listen\": \"\",")
    print("       \"port\": 443,")
    print("       \"protocol\": \"hysteria2\",")
    print("       \"settings\": \"{\\\"password\\\": \\\"your_password\\\", \\\"up\\\": 200, \\\"down\\\": 200}\",")
    print("       \"streamSettings\": \"{\\\"network\\\": \\\"udp\\\", \\\"security\\\": \\\"tls\\\", \\\"tlsSettings\\\": {\\\"certificates\\\": [{\\\"certificateFile\\\": \\\"/path/to/cert.crt\\\", \\\"keyFile\\\": \\\"/path/to/key.key\\\"}]}}\"")
    print("     }'")
    print("==================================================\n")
...
def main() -> None:
    ...
    parser.add_argument(
        "--keys",
        required=True,
        help="Path to text file with vless://, trojan://, hysteria:// and hy2:// links",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=200,
        help="Default bandwidth (Mbps) for Hysteria/Hysteria2 if not specified in link",
    )
    parser.add_argument(
        "--timeout", type=int, default=30, help="Seconds per test category (sites/youtube/torrent)"
    )
    ...
    parser.add_argument(
        "--no-shuffle-output", action="store_true", help="Do not shuffle nodes in exported config"
    )
    parser.add_argument("--export-singbox", help="Path to export sing-box configuration")
    parser.add_argument("--show-inbound", action="store_true", help="Show Hysteria2 inbound configuration example")
    ...
    try:
        ...
        print(f"[!] Warning: {e}")
        print("[!] Some tests may fail or be skipped if binaries are missing.")
