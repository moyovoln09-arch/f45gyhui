...
def parse_link(link: str, default_bandwidth: int = 200) -> Tuple[str, Dict]:
    ...
    if scheme == "vless":
        ...
        outbound = {
            ...
            "server": server,
            "server_port": port,
            "auth_str": unquote(parsed.username) if parsed.username else (qs.get("auth") or ""),
            "up_mbps": to_int(qs.get("upmbps") or qs.get("up_mbps") or qs.get("up"), default_bandwidth),
            "down_mbps": to_int(qs.get("downmbps") or qs.get("down_mbps") or qs.get("down"), default_bandwidth),
        }
        if qs.get("obfs"):
            outbound["obfs"] = qs["obfs"]

    security = qs.get("security", "")
    # Hysteria2 and Hysteria always use TLS in sing-box, but we can set SNI and insecure
    if security in {"tls", "reality"} or scheme in {"hysteria2", "hy2", "hysteria"}:
        tls = {
            "enabled": True,
...
def main() -> None:
    ...
    parser.add_argument("--hy-bandwidth", type=int, default=200, help="Default bandwidth for Hysteria/Hysteria2 in Mbps")
