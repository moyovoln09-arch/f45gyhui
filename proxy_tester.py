...
def parse_link(link: str) -> Tuple[str, Dict]:
    ...
    if scheme == "vless":
        ...
        outbound = {
            ...
            "up_mbps": to_int(qs.get("up_mbps") or qs.get("up"), 200),
            "down_mbps": to_int(qs.get("down_mbps") or qs.get("down"), 200),
        }
        if qs.get("obfs"):
            outbound["obfs"] = {
                "type": qs.get("obfs"),
    ...
    if security in {"tls", "reality"} or scheme == "hysteria2":
        tls = {
            ...
            "insecure": (qs.get("insecure") or qs.get("allowInsecure") or "0") in {"1", "true", "True"},
        }
        if security == "reality":
            tls["reality"] = {
                "enabled": True,
