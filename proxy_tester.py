...
def parse_link(link: str) -> Tuple[str, Dict]:
    ...
    if scheme == "vless":
        ...
        outbound = {
            ...
            "up_mbps": int(qs.get("up", 200)),
            "down_mbps": int(qs.get("down", 200)),
        }
        if qs.get("obfs"):
            outbound["obfs"] = {
                "type": qs["obfs"],
...
def export_balanced_config(results: List[TestResult], top_n: int, output_path: Path) -> None:
    ranked = [r for r in sorted(results, key=lambda r: r.total_score, reverse=True) if r.working]
    top_keys = ranked[:top_n]
    # Shuffle top keys to randomize "first, second, third" position in the exported config
    random.shuffle(top_keys)

    outbounds = []
    outbound_tags = []

    for i, res in enumerate(top_keys):
    ...
    manual_selector = {
        "type": "selector",
        "tag": "manual-proxy",
        "outbounds": ["best-latency"] + outbound_tags
    }
    ...
    config = {
        ...
        "route": {
            ...
            "rules": [
                ...
                # This rule uses the random balancer for all other traffic
                # If the user wants to use manual selector, they'd have to change this.
                # But to satisfy the "randomly choosing between nodes" request,
                # we make it the primary routing mechanism.
                {"action": "route", "balancer": "random-balancer"}
            ],
            "final": "manual-proxy",
    ...
    print(f"[+] Balanced config exported to: {output_path}")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Test VLESS/Trojan/Hysteria2 keys and rank best 5 by average score (sites + YouTube + torrent + stability)."
    )
    parser.add_argument("--keys", required=True, help="Path to text file with vless:// and trojan:// links")
    ...
    parser.add_argument("--shuffle-output", action="store_true", help="Shuffle top-N keys in the final display")
    parser.add_argument("--export-config", help="Path to export a balanced sing-box config using top keys")
    ...
    print_top(results, args.top, shuffle_output=args.shuffle_output)
    save_report(results, Path(args.report))

    if args.export_config:
        export_balanced_config(results, args.top, Path(args.export_config))
