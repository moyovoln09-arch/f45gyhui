    ...
    if scheme in {"hysteria2", "hy2"}:
        outbound = {
            ...
            "tls": {
                ...
                "insecure": (qs.get("insecure") or qs.get("allowInsecure", "0")) in {"1", "true", "True"},
            },
            "up_mbps": to_int(qs.get("up") or qs.get("up_mbps"), 200),
            "down_mbps": to_int(qs.get("down") or qs.get("down_mbps"), 200),
        }
        if qs.get("fp"):
            outbound["tls"]["utls"] = {"enabled": True, "fingerprint": qs["fp"]}
        if qs.get("obfs"):
            outbound["obfs"] = {
                "type": qs["obfs"],
...
def export_singbox_config(
    results: List[TestResult], top_n: int, export_path: Path, shuffle_output: bool = True
) -> None:
    working = [r for r in results if r.working]
    top_working = sorted(working, key=lambda r: r.total_score, reverse=True)[:top_n]
    ...
    nodes = []
    for idx, res in enumerate(top_working, start=1):
        try:
            _, outbound = parse_link(res.key)
            tag = f"node-{idx}-{outbound['type']}"
            outbound["tag"] = tag
            nodes.append((outbound, tag))
        except Exception:
            continue
    ...
    outbounds = [n[0] for n in nodes]
    proxy_tags = [n[1] for n in nodes]
    ...
    full_outbounds = [selector, loadbalance, urltest] + outbounds + [
        {"type": "direct", "tag": "direct"},
    ...
    config = {
        ...
        "dns": {
            "servers": [
                {"tag": "google", "address": "8.8.8.8", "detour": "select"},
    ...
    print(f"[+] Exported sing-box config to: {export_path}")
...
def main() -> None:
    ...
    parser.add_argument("--keys", required=True, help="Path to text file with proxy links")
    parser.add_argument("--top", type=int, default=5, help="How many keys to show in top")
    parser.add_argument("--timeout", type=int, default=30, help="Seconds per test category")
    ...
    parser.add_argument("--export-singbox", help="Path to export best working nodes as sing-box config")
    parser.add_argument("--no-shuffle-output", action="store_true", help="Disable shuffling of output nodes")
    ...
    if args.export_singbox:
        export_singbox_config(
            results, args.top, Path(args.export_singbox), shuffle_output=not args.no_shuffle_output
        )
