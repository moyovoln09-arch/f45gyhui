...
def parse_link(link: str) -> Tuple[str, Dict]:
    ...
    if scheme == "vless":
        ...
        outbound = {
            ...
            "server": server,
            "server_port": port,
            "up_mbps": int(qs.get("up_mbps") or qs.get("up") or 200),
            "down_mbps": int(qs.get("down_mbps") or qs.get("down") or 200),
        }
        if parsed.username:
            outbound["password"] = unquote(parsed.username)

        if qs.get("obfs"):
            outbound["obfs"] = {
                "type": qs["obfs"],
                "password": qs.get("obfs-password") or qs.get("obfs_password") or ""
            }
...
def export_singbox_config(results: List[TestResult], export_path: Path, top_n: int, shuffle_output: bool = True) -> None:
    ...
    # Shuffle proxy tags for the selector to avoid bias toward the first entries
    selector_proxy_tags = list(proxy_tags)
    random.shuffle(selector_proxy_tags)

    # Define selectors and special outbounds
    special_outbounds = [
        {
            "type": "selector",
            "tag": "select",
            "outbounds": ["random", "auto"] + selector_proxy_tags,
        },
        {
            "type": "load-balance",
            "tag": "random",
    ...
    config = {
        ...
        "outbounds": special_outbounds + outbounds,
        "route": {
            "rules": [
