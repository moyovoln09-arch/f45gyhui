...
def parse_link(link: str) -> Tuple[str, Dict]:
    ...
    if scheme not in {"vless", "trojan", "hy2", "hysteria2"}:
    ...
    if security in {"tls", "reality"} or scheme in {"hy2", "hysteria2"}:
        tls = {
            "enabled": True,
            "server_name": qs.get("sni") or qs.get("host") or server,
            "insecure": qs.get("allowInsecure", "0") in {"1", "true", "True"} or qs.get("insecure", "0") in {"1", "true", "True"},
        }
        if security == "reality":
            tls["reality"] = {
                "enabled": True,
