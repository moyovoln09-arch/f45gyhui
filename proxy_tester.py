...
def parse_link(link: str) -> Tuple[str, Dict]:
    ...
    if security in {"tls", "reality"} and scheme in {"vless", "trojan"}:
        tls = {
            "enabled": True,
        ...
        if not parsed.username:
            raise ValueError("Invalid Hysteria2 link: no password")
        ...
        up = qs.get("up_mbps") or qs.get("up") or "200"
        down = qs.get("down_mbps") or qs.get("down") or "200"
        ...
        outbound = {
            ...
            "server_port": port,
            "password": unquote(parsed.username),
            "up_mbps": to_int(up, 200),
            "down_mbps": to_int(down, 200),
            "tls": {
                "enabled": True,
                "server_name": qs.get("sni") or server,
                "insecure": qs.get("insecure") in {"1", "true", "True"} or qs.get("allowInsecure") in {"1", "true", "True"},
            },
        }
        if qs.get("obfs"):
            outbound["obfs"] = {
...
def export_singbox_config(results: List[TestResult], output_path: Path, shuffle_output: bool = True) -> None:
    nodes = []
    working_results = [r for r in results if r.working]

    if shuffle_output:
        random.shuffle(working_results)

    for i, res in enumerate(working_results, start=1):
        try:
    ...
    print(f"[+] Sing-box config exported to: {output_path}")
...
def main() -> None:
    ...
    parser.add_argument("--export-singbox", help="Path to export best keys as sing-box config")
    ...
    if args.export_singbox:
        ...
        export_singbox_config(ranked[:args.top], Path(args.export_singbox), not args.no_shuffle_output)
