...
def parse_link(link: str) -> Tuple[str, Dict]:
    ...
    if scheme == "vless":
        ...
        outbound = {
            ...
            "up_mbps": to_int(qs.get("up", qs.get("up_mbps")), 200),
            "down_mbps": to_int(qs.get("down", qs.get("down_mbps")), 200),
        }
        if qs.get("obfs"):
            outbound["obfs"] = {
                "type": qs.get("obfs"),
        ...
        tls = {
            ...
            "insecure": qs.get("insecure", qs.get("allowInsecure", "0")) in {"1", "true", "True"},
        }
        outbound["tls"] = tls
...
def export_singbox_config(results: List[TestResult], export_path: Path, top_n: int, shuffle_output: bool = True) -> None:
    ...
    config = {
        ...
        "dns": {
            "servers": [
                {"tag": "dns-remote", "address": "https://8.8.8.8/dns-query"},
