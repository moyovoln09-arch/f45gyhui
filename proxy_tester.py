...
def parse_link(link: str, default_hy2_bw: int = 200) -> Tuple[str, Dict]:
    ...
    if scheme == "vless":
    ...
    elif scheme in {"hysteria2", "hy2", "hysteria"}:
        ...
        if scheme == "hysteria":
            outbound = {
                ...
                "server": server,
                "server_port": port,
                "auth_str": unquote(parsed.username) if parsed.username else "",
                "up_mbps": to_int(qs.get("up") or qs.get("up_mbps"), default_hy2_bw),
                "down_mbps": to_int(qs.get("down") or qs.get("down_mbps"), default_hy2_bw),
            }
            if qs.get("obfs") and qs["obfs"] != "none":
...
def evaluate_key(key: str, timeout_s: int, sites: List[str], magnet: str, yt_url: str, default_hy2_bw: int = 200) -> TestResult:
    with tempfile.TemporaryDirectory(prefix="keytest_") as td:
        td_path = Path(td)
        cfg_path = td_path / "config.json"

        try:
            protocol, outbound = parse_link(key, default_hy2_bw=default_hy2_bw)
...
def export_singbox_config(
    results: List[TestResult], top_n: int, output_path: Path, shuffle_output: bool = True
) -> None:
    ranked = sorted(results, key=lambda r: r.total_score, reverse=True)
    ...
    for r in top_results:
        if r.outbound:
            nodes_to_export.append(r.outbound)
...
def main() -> None:
    ...
    parser.add_argument("--keys", required=True, help="Path to text file with vless://, trojan://, or hy2:// links")
    parser.add_argument("--top", type=int, default=5, help="How many keys to show in top")
    parser.add_argument("--hy2-bandwidth", type=int, default=200, help="Default bandwidth in Mbps for Hysteria2 nodes (used if not in link)")
    ...
    for i, key in enumerate(keys, start=1):
        ...
        res = evaluate_key(key, args.timeout, DEFAULT_SITES, args.magnet, args.youtube_url, args.hy2_bandwidth)
        results.append(res)
    ...
    if args.export_singbox:
        export_singbox_config(
            results, args.top, Path(args.export_singbox), not args.no_shuffle_output
        )
