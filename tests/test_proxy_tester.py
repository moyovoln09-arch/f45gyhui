...
def test_parse_vless_extra_params():
    link = "vless://uuid@example.com:443?security=tls&sni=mysni&fp=chrome&insecure=1"
    protocol, outbound = parse_link(link)
...
def test_parse_trojan_extra_params():
    link = "trojan://pass@example.com:443?security=tls&sni=mysni&fp=safari&allowInsecure=1"
    protocol, outbound = parse_link(link)
    ...
    assert outbound["tls"]["insecure"] is True
    assert outbound["tls"]["utls"]["enabled"] is True
    assert outbound["tls"]["utls"]["fingerprint"] == "safari"
