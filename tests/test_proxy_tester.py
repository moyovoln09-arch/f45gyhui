...
def test_parse_hysteria2_bandwidth_robustness():
    # Test with invalid bandwidth strings to ensure they fallback to 200
    link = "hy2://pass@example.com:443?up=invalid&down="
    _, outbound = parse_link(link)
    assert outbound["up_mbps"] == 200
    assert outbound["down_mbps"] == 200

    # Test with valid bandwidth strings
    link = "hy2://pass@example.com:443?up=150mbps&down=300"
    _, outbound = parse_link(link)
    assert outbound["up_mbps"] == 150
    assert outbound["down_mbps"] == 300

def test_parse_vless_reality_fp():
    link = "vless://uuid@example.com:443?security=reality&sni=example.com&fp=chrome&pbk=pubkey&sid=shortid"
    _, outbound = parse_link(link)
    assert outbound["tls"]["utls"]["enabled"] is True
    assert outbound["tls"]["utls"]["fingerprint"] == "chrome"

def test_export_singbox_loadbalance_and_default(tmp_path):
    results = [
        TestResult(key=f"hy2://pass@s{i}.com:443", protocol="hy2", working=True, startup_error=None,
                   website_success_ratio=1.0, website_avg_latency_ms=10.0, website_details=[],
                   youtube_ok=True, youtube_speed_mbps=1.0, youtube_bytes=1,
    ...
    output_file = tmp_path / "config_lb.json"
    export_singbox_config(results, output_file, shuffle_output=True)

    config = json.loads(output_file.read_text())
    outbounds = config["outbounds"]

    # Check for 'random' loadbalance outbound
    lb = next((o for o in outbounds if o["tag"] == "random"), None)
    assert lb is not None
    assert lb["type"] == "loadbalance"
    assert lb["strategy"] == "random"
    assert len(lb["outbounds"]) == 3

    # Check 'select' outbound default
    selector = next((o for o in outbounds if o["tag"] == "select"), None)
