...
def test_parse_hysteria_basic():
    link = "hysteria://auth_string@example.com:443?up=150&down=150&obfs=obfspass&sni=mysni"
    scheme, outbound = parse_link(link)
    assert scheme == "hysteria"
    assert outbound["type"] == "hysteria"
    assert outbound["server"] == "example.com"
    assert outbound["server_port"] == 443
    assert outbound["auth_str"] == "auth_string"
    assert outbound["up_mbps"] == 150
    assert outbound["down_mbps"] == 150
    assert outbound["obfs"] == "obfspass"
    assert outbound["tls"]["server_name"] == "mysni"

def test_parse_hysteria_default_bandwidth():
    link = "hysteria://auth_string@example.com:443"
    scheme, outbound = parse_link(link, default_bandwidth=500)
